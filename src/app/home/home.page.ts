...
export class HomePage {
  ...
  conectar() {
    ...
    // Validación: el nombre de usuario no puede exceder los 50 caracteres
    if (this.user.usuario.length > 20) {
      this.msj = 'El nombre de usuario no puede exceder los 20 caracteres.';
      return;
    }

    // Validación: la contraseña no puede exceder los 50 caracteres
    if (this.user.password.length > 20) {
      this.msj = 'La contraseña no puede exceder los 20 caracteres.';
      return;
    }
  ...
  // Limpia las credenciales cuando se regresa a esta página
  ionViewWillEnter() {
